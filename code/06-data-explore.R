...
main_df$drought_day<- (-1)*main_df$drought_day

#Question 1: How persistent are drought-days?

main_df<-main_df %>%
  group_by(GeoFIPS) %>%
  dplyr::mutate(lag1 = dplyr::lag(drought_day,n=1,default=NA)) %>%
  dplyr::mutate(lag2 = dplyr::lag(drought_day,n=2,default=NA)) %>%
  dplyr::mutate(lag3 = dplyr::lag(drought_day,n=3,default=NA)) %>%
  dplyr::mutate(lag4 = dplyr::lag(drought_day,n=4,default=NA)) %>%
  dplyr::mutate(lag5 = dplyr::lag(drought_day,n=5,default=NA)) %>%
  as.data.frame()
...
#Scatter, partialling out fixed effects
...
(ggplot(main_df, aes(x=drought_day_demean,y=farm_income_demean)) +
    geom_point(alpha = 0.4) +
    stat_summary_bin(fun='mean', bins=20,
                     color='orange', size=2, geom='point'))

(ggplot(main_df, aes(x=drought_day_demean,y=nonfarm_income_demean)) +
    geom_point(alpha = 0.4) +
    stat_summary_bin(fun='mean', bins=20,
                     color='orange', size=2, geom='point'))
